...
<script type="application/ld+json">
{
  ...
  "description": "<?php echo e($product->meta_description ?? strip_tags($product->description)); ?>",
  "sku": "<?php echo e($product->slug); ?>",
...
<?php
    ...
    $textColor = $isDark ? 'text-[#d4af37]' : 'text-gray-900';
    $mutedColor = $isDark ? 'text-[#f3d9ff]/80' : 'text-gray-500';
    $borderColor = $isDark ? 'border-[#d4af37]' : 'border-gray-300';
    $bgColor = $isDark ? 'bg-[#1a0023]' : 'bg-[#f2f2f2]';
    $accentColor = $isDark ? 'text-[#f3d9ff]' : 'text-gray-900';
    $accentBg = $isDark ? 'bg-[#d4af37]' : 'bg-black';
    $cardBg = $isDark ? 'bg-[#2B003A]/50' : 'bg-white';
...
<div class="<?php echo e($bgColor); ?> selection:bg-black selection:text-white transition-colors duration-700">
    <div class="max-w-[1320px] mx-auto px-4 sm:px-6 lg:px-10 pt-8 pb-20">

        <nav class="flex items-center gap-2 text-[10px] font-bold tracking-[0.24em] uppercase <?php echo e($mutedColor); ?> mb-6 overflow-x-auto whitespace-nowrap hide-scrollbar" aria-label="Breadcrumb">
            ...
            <?php if($product->category): ?>
                <a href="<?php echo e(route('front.products.index', ['category' => $product->category->slug])); ?>" class="hover:<?php echo e($textColor); ?> transition-colors"><?php echo e($product->category->name); ?></a>
                <span class="opacity-30">/</span>
            <?php endif; ?>
            <span class="<?php echo e($textColor); ?>"><?php echo e($product->name); ?></span>
        </nav>

        <?php if(!$isDark): ?>
        <section class="mb-6 border <?php echo e($borderColor); ?> bg-white overflow-hidden">
            <img src="<?php echo e($couponBanner); ?>" alt="AVNEE COLLECTIONS Coupon Banner" class="w-full h-auto object-cover" />
        </section>
        <?php endif; ?>

        <?php if($isDark): ?>
        <section class="mb-10 border <?php echo e($borderColor); ?> bg-gradient-to-r from-[#2b003a] via-[#3d0a43] to-[#2b003a] px-4 py-3">
        ...
        <div class="grid grid-cols-1 xl:grid-cols-[72px_minmax(0,1fr)_360px] gap-5 xl:gap-7 items-start">
            ...
            <div class="order-2 xl:order-1 flex xl:flex-col gap-2 overflow-x-auto xl:overflow-visible pb-1 xl:pb-0">
                <button class="detail-thumb shrink-0 w-[64px] xl:w-full aspect-[3/4] overflow-hidden border <?php echo e($isDark ? 'border-[#d4af37]' : 'border-black'); ?> bg-white" onclick="changeImage('<?php echo e(asset('storage/' . $product->image)); ?>', this)">
                    <img src="<?php echo e(asset('storage/' . $product->image)); ?>" alt="<?php echo e($product->name); ?> thumbnail" class="w-full h-full object-cover object-top" />
                </button>
                <?php $__currentLoopData = $product->images; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $img): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <?php if($img->path !== $product->image): ?>
                    <button class="detail-thumb shrink-0 w-[64px] xl:w-full aspect-[3/4] overflow-hidden border <?php echo e($borderColor); ?> bg-white" onclick="changeImage('<?php echo e(asset('storage/' . $img->path)); ?>', this)">
                        <img src="<?php echo e(asset('storage/' . $img->path)); ?>" alt="<?php echo e($product->name); ?> thumbnail" class="w-full h-full object-cover object-top" />
                    </button>
                    <?php endif; ?>
            ...
            <div class="order-1 xl:order-2">
                <div class="relative bg-[#d8d8d8] border <?php echo e($borderColor); ?> w-full max-w-[640px] xl:max-w-[700px] mx-auto xl:mx-0 min-h-[390px] sm:min-h-[520px] xl:min-h-[590px] flex items-center justify-center overflow-hidden">
                    <img id="main-product-image" src="<?php echo e(asset('storage/' . $product->image)); ?>" alt="<?php echo e($product->name); ?>" class="max-h-[92%] max-w-[92%] object-contain transition-all duration-500 ease-out" />
                    <?php if($activeDiscount > 0): ?>
                        <div class="absolute top-4 left-4 <?php echo e($accentBg); ?> text-white text-[10px] font-bold px-3 py-1.5 tracking-[0.18em] uppercase">
                            <?php echo e(round($activeDiscount)); ?>% Off
                        </div>
                    <?php endif; ?>
                    ...
                    <span id="gallery-counter" class="absolute bottom-3 right-3 px-2 py-1 text-[10px] font-semibold rounded bg-black/40 text-white">1 / <?php echo e($galleryCount); ?></span>
                </div>
            </div>

            <div class="order-3 <?php echo e($textColor); ?> xl:sticky xl:top-20 <?php echo e($isDark ? 'border border-[#4f006a] bg-[#230030]/55 p-4 sm:p-5' : ''); ?> <?php echo e(!$isDark ? 'bg-transparent' : ''); ?>">
                <div class="flex items-start justify-between gap-3 mb-3">
                    <h1 class="text-2xl leading-tight font-semibold tracking-tight max-w-[85%]"><?php echo e($product->name); ?></h1>
                    <div class="flex items-center gap-2 pt-1">
                        <button type="button" onclick="toggleWishlist(<?php echo e($product->id); ?>)" class="p-1.5 rounded-full hover:bg-black/5 <?php echo e($product->isWishlisted ? 'text-red-500' : ''); ?>" aria-label="Toggle wishlist">
                            <svg class="w-4 h-4 <?php echo e($product->isWishlisted ? 'fill-current' : ''); ?>" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"></path></svg>
                        </button>
                ...
                <p class="text-[10px] <?php echo e($mutedColor); ?> tracking-[0.22em] uppercase mb-1"><?php echo e($isDark ? ('AVNEE ' . strtoupper($categoryTitle) . ' Archive') : strtoupper($categoryTitle)); ?></p>
                <p class="text-[12px] <?php echo e($mutedColor); ?> mb-1">Style No <?php echo e($product->sku ?? 'AVN' . str_pad($product->id, 6, '0', STR_PAD_LEFT)); ?></p>

                <div class="mb-5">
                    <?php if($strikePrice > $activePrice): ?>
                        <p class="text-xs <?php echo e($mutedColor); ?> line-through mb-1">MRP ₹<?php echo e(number_format($strikePrice, 0)); ?></p>
                    <?php endif; ?>
                    <p class="text-[28px] font-bold leading-none">₹<?php echo e(number_format($activePrice, 0)); ?></p>
                    <p class="text-[11px] <?php echo e($mutedColor); ?> mt-1">Inclusive of all taxes</p>
                </div>
                ...
                <form action="<?php echo e(route('cart.add')); ?>" method="POST" id="add-to-cart-form">
                    ...
                    <div class="mb-5">
                        <div class="flex items-center justify-between mb-3">
                            ...
                            <button type="button" id="size-guide-trigger" class="text-xs underline underline-offset-2 <?php echo e($accentColor); ?> hover:opacity-70 transition-opacity">Size Guide</button>
                        </div>
                        <div class="flex flex-wrap gap-2">
                            <?php $__currentLoopData = $product->variants->unique('size'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $variant): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <?php
                            ...
                            <label class="cursor-pointer">
                                <input type="radio" name="size" value="<?php echo e($variant->size); ?>" data-variant-id="<?php echo e($stockVariant?->id); ?>" class="peer sr-only" <?php echo e($index === 0 ? 'required' : ''); ?> <?php echo e($isDisabled ? 'disabled' : ''); ?> />
                                <span class="inline-flex items-center justify-center min-w-[84px] h-9 px-3 rounded-full border <?php echo e($borderColor); ?> text-xs font-medium peer-checked:bg-black peer-checked:text-white <?php echo e($isDark ? 'peer-checked:bg-[#d4af37] peer-checked:text-[#1a0023]' : ''); ?> <?php echo e($isDisabled ? 'opacity-40 cursor-not-allowed pointer-events-none' : ''); ?>">
                        ...
                        <p class="text-[11px] <?php echo e($mutedColor); ?> mt-2">Select your preferred size before adding to cart.</p>
                    </div>
                    <?php endif; ?>

                    <?php if($isKidsProduct): ?>
                    <div class="mb-5">
                        <div class="flex items-center justify-between mb-3">
                            <p class="text-sm font-semibold">Kids Age Sizes</p>
                            <span class="text-[11px] <?php echo e($mutedColor); ?>">Tap to match size</span>
                        </div>
                        <div class="flex flex-wrap gap-2">
                            <?php $__currentLoopData = $kidsAgeSizes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ageSize): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <?php
                                $ageKey = strtolower(preg_replace('/\s+/', '', (string) $ageSize));
                                $ageDisabled = !$availableSizeKeys->contains($ageKey);
                            ?>
                            <button type="button" class="age-size-chip inline-flex items-center justify-center min-w-[88px] h-8 px-3 rounded-full border <?php echo e($borderColor); ?> text-[11px] font-medium <?php echo e($isDark ? 'hover:bg-[#d4af37]/15' : 'hover:bg-gray-100'); ?> transition-colors <?php echo e($ageDisabled ? 'opacity-40 cursor-not-allowed pointer-events-none' : ''); ?>" data-age-size="<?php echo e($ageSize); ?>">
                                <?php echo e($ageSize); ?>
                    ...
                    <div class="mb-2 flex items-center gap-2">
                        <div class="relative flex-1">
                            <input
                                ...
                                class="w-full h-9 border <?php echo e($borderColor); ?> px-3 text-xs <?php echo e($isDark ? 'bg-[#230030] text-white' : 'bg-white text-gray-900'); ?>"
                            />
                            <div
                                id="delivery-pincode-suggestions"
                                class="hidden absolute left-0 right-0 top-[calc(100%+4px)] z-30 border <?php echo e($isDark ? 'border-[#4f006a] bg-[#1f0028] text-white' : 'border-gray-300 bg-white text-gray-900'); ?> max-h-52 overflow-y-auto shadow-lg"
                            ></div>
                        </div>
                        <button type="button" id="delivery-pincode-check-btn" class="h-9 px-3 <?php echo e($isDark ? 'bg-[#d4af37] text-[#1a0023]' : 'bg-[#122f2f] text-white'); ?> text-[11px] font-semibold">Check Service</button>
                    </div>
                    ...
                    <div class="mb-6 space-y-2">
                        <div class="flex items-stretch gap-2 h-11">
                            <div class="flex items-center border <?php echo e($borderColor); ?> w-[96px] shrink-0 <?php echo e($isDark ? 'bg-[#230030]' : 'bg-white'); ?>">
                            ...
                            <button type="button" onclick="addToCartWithDrawer()" class="flex-1 h-full border <?php echo e($isDark ? $borderColor : 'border-black'); ?> <?php echo e($isDark ? 'bg-[#2B003A] text-white' : 'bg-black text-white'); ?> text-[12px] font-semibold uppercase tracking-wide">Add To Cart • ₹<?php echo e(number_format($activePrice, 0)); ?></button>
                        </div>
                        <button type="button" onclick="buyNow()" class="w-full h-11 <?php echo e($isDark ? 'bg-[#d4af37] text-[#1a0023]' : 'bg-white border border-black text-black'); ?> text-[12px] font-semibold uppercase tracking-wide">Buy It Now</button>
                    </div>

                    <div class="mb-6 pt-2">
                        <div class="grid grid-cols-2 gap-x-6 gap-y-3 text-[12px] leading-snug <?php echo e($mutedColor); ?>">
                            ...
                            <?php if($product->show_returnable_badge ?? true): ?>
                            <div class="flex items-center gap-2"><span class="text-base">↻</span><span><?php echo e($isReturnable ? 'This product is returnable' : 'This product is not returnable'); ?></span></div>
                            <?php endif; ?>
                            <?php if($product->show_free_shipping_badge ?? true): ?>
                ...
                <div class="space-y-px border-t <?php echo e($borderColor); ?> pt-2">
                    <details class="group border-b <?php echo e($borderColor); ?>" open>
                        ...
                        <div class="pb-5 text-[13px] leading-relaxed <?php echo e($mutedColor); ?>">
                            <?php echo $product->description; ?>

                        </div>
                    </details>

                    <details class="group border-b <?php echo e($borderColor); ?>">
                        <summary class="flex justify-between items-center py-4 cursor-pointer list-none text-[18px] font-medium">
                    ...
                    <div class="pt-8 pb-4 text-[12px] leading-relaxed <?php echo e($mutedColor); ?> border-b <?php echo e($borderColor); ?>">
                        Main image is shown first, and all remaining photos from this product folder are available in the gallery below.
                    </div>
                    <?php if($product->category): ?>
                    <a href="<?php echo e(route('front.products.index', ['category' => $product->category->slug])); ?>" class="block pt-4 text-[11px] font-bold uppercase tracking-[0.22em] <?php echo e($accentColor); ?> hover:opacity-70 transition-opacity">
                        Back to <?php echo e($product->category->name); ?>
        ...
        <div class="mt-16 pt-10 border-t <?php echo e($borderColor); ?>">
            ...
            <div class="grid grid-cols-2 lg:grid-cols-5 gap-5">
                ...
                <div class="group relative flex flex-col">
                    <a href="<?php echo e(route('front.product.detail', $related->slug ?? $related->id)); ?>" class="relative aspect-[3/4] overflow-hidden mb-6 block shadow-xl transition-all duration-700">
                        <img src="<?php echo e(asset('storage/' . $related->image)); ?>" alt="<?php echo e($related->name); ?>" class="w-full h-full object-cover object-top filter grayscale-[30%] group-hover:grayscale-0 transition-all duration-[2s] group-hover:scale-105" />
                        <?php if($related->discount > 0): ?>
                            <div class="absolute top-4 left-4 bg-white/90 backdrop-blur-md text-black text-[9px] font-bold px-3 py-1.5 tracking-widest uppercase">-<?php echo e(round($related->discount)); ?>%</div>
                        <?php endif; ?>
                    </a>
                    <div class="space-y-2">
...
<div id="size-guide-panel" class="fixed inset-0 z-50 hidden" aria-hidden="true">
    ...
    <aside id="size-guide-drawer" class="absolute right-0 top-0 h-full w-full max-w-md <?php echo e($isDark ? 'bg-[#1f0028] text-white border-l border-[#4f006a]' : 'bg-white text-gray-900 border-l border-gray-200'); ?> translate-x-full transition-transform duration-300 shadow-2xl overflow-y-auto">
        <div class="p-6 border-b <?php echo e($borderColor); ?> flex items-center justify-between">
        ...
        <div class="p-6 space-y-5 text-sm <?php echo e($mutedColor); ?>">
            <p>Choose the closest chest measurement for the best fit. For layered styling, select one size up.</p>
            <div class="border <?php echo e($borderColor); ?> overflow-hidden">
                <table class="w-full text-left text-xs">
                    <thead class="<?php echo e($isDark ? 'bg-[#2B003A]' : 'bg-gray-50'); ?> <?php echo e($textColor); ?> uppercase tracking-[0.14em]">
                        <tr>
                    ...
                    <tbody>
                        <tr class="border-t <?php echo e($borderColor); ?>"><td class="px-4 py-3">1-2Y</td><td class="px-4 py-3">Toddler</td><td class="px-4 py-3">20-21</td></tr>
                        <tr class="border-t <?php echo e($borderColor); ?>"><td class="px-4 py-3">2-4Y</td><td class="px-4 py-3">Kids</td><td class="px-4 py-3">22-23</td></tr>
                        <tr class="border-t <?php echo e($borderColor); ?>"><td class="px-4 py-3">4-6Y</td><td class="px-4 py-3">Kids</td><td class="px-4 py-3">24-25</td></tr>
                        <tr class="border-t <?php echo e($borderColor); ?>"><td class="px-4 py-3">6-8Y</td><td class="px-4 py-3">Juniors</td><td class="px-4 py-3">26-27</td></tr>
                        <tr class="border-t <?php echo e($borderColor); ?>"><td class="px-4 py-3">8-10Y</td><td class="px-4 py-3">Juniors</td><td class="px-4 py-3">28-29</td></tr>
                    </tbody>
                </table>
            </div>
...
<aside id="detail-cart-drawer" class="fixed top-0 right-0 h-full w-full max-w-[430px] bg-white text-gray-900 z-[130] shadow-2xl translate-x-full transition-transform duration-300">
    <div class="h-full flex flex-col">
        ...
        <div class="p-4 space-y-3">
            ...
            <a href="<?php echo e(route('front.checkout.index')); ?>" class="block w-full h-12 bg-black text-white text-center leading-[48px] text-[18px] font-semibold uppercase">Checkout</a>
            <a href="<?php echo e(route('front.cart.index')); ?>" class="block w-full h-12 border border-black text-center leading-[48px] text-[18px] font-semibold uppercase">View Cart</a>
            <p class="text-[13px] leading-relaxed text-gray-700 text-center">By clicking on checkout you are agreeing to <a href="<?php echo e(route('front.page', ['slug' => 'return-policy'])); ?>" class="underline">Return Policy</a>.</p>
            <div>
...
<script>
    function changeImage(src, element) {
        ...
        buttons.forEach(btn => btn.classList.remove('<?php echo e($isDark ? "border-brand-gold" : "border-mulberry"); ?>'));
        buttons.forEach(btn => btn.classList.add('<?php echo e($borderColor); ?>'));
        element.classList.remove('<?php echo e($borderColor); ?>');
        element.classList.add('<?php echo e($isDark ? "border-brand-gold" : "border-mulberry"); ?>');
    ...
    function shareProduct() {
        if (navigator.share) {
            navigator.share({
                title: '<?php echo e($product->name); ?> — AVNEE ARCHIVE',
    ...
    function detailCartUpdateUrl(itemId) {
        return <?php echo json_encode(route('front.cart.update', ['id' => '__ID__']), 512) ?>.replace('__ID__', String(itemId));
    }

    function detailCartRemoveUrl(itemId) {
        return <?php echo json_encode(route('front.cart.remove', ['id' => '__ID__']), 512) ?>.replace('__ID__', String(itemId));
    }

    function fetchDetailCartSummary() {
        return fetch(<?php echo json_encode(route('front.cart.summary'), 15, 512) ?>, {
            method: 'GET',
            headers: {
    ...
    (function () {
        ...
        const getActiveIndex = () => {
            return Math.max(0, thumbs.findIndex((btn) => btn.classList.contains('<?php echo e($isDark ? "border-brand-gold" : "border-mulberry"); ?>')));
        };
    ...
    (function () {
        ...
        const renderSuggestions = async (digits) => {
            ...
            list.innerHTML = matches.map((pin) => {
                ...
                return (
                    '<button type="button" class="w-full text-left px-3 py-2 text-xs hover:' + (<?php echo json_encode($isDark ? 'bg-[#2b003a]' : 'bg-gray-100', 15, 512) ?>) + '" data-pin="' + pin + '">' +
...
<?php echo $__env->make('layouts.front.' . $theme, array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\My projects\CP\avnee\avnee_files\resources\views/front/product/detail.blade.php ENDPATH**/ ?>