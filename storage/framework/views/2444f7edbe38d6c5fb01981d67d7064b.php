...
<section class="mt-10 mb-20 px-4 sm:px-6 lg:px-8">
    <div class="max-w-[1320px] mx-auto bg-[#522B6B] border border-[#522B6B]/50 px-4 sm:px-6 lg:px-10 py-8 sm:py-10 lg:py-12">
        <h2 class="text-center font-heading text-3xl sm:text-4xl lg:text-5xl <?php echo e($isJewellery ? 'text-[#c7a4cc]' : 'text-[#2b2320]'); ?> leading-tight mb-8 sm:mb-10">Read Our Blog</h2>

        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
            <?php $__currentLoopData = $cards; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $card): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <article class="bg-transparent">
                <div class="aspect-[16/10] overflow-hidden <?php echo e($isJewellery ? 'bg-[#44206f]' : 'bg-white'); ?> rounded-sm">
                    <img src="<?php echo e($card['image']); ?>" alt="<?php echo e($card['title']); ?>" class="w-full h-full object-cover transition-transform duration-700 hover:scale-105" />
                </div>
                <div class="pt-4 sm:pt-5">
                    <h3 class="font-heading text-xl sm:text-2xl lg:text-[38px] leading-[1.12] text-[#2e2624]"><?php echo e($card['title']); ?></h3>
                    <p class="mt-3 text-[10px] sm:text-[11px] tracking-[0.14em] text-[#6e6663] uppercase"><?php echo e($card['meta']); ?></p>
                    <p class="mt-4 text-sm sm:text-base lg:text-[20px] leading-[1.65] text-[#4f4643]"><?php echo e($card['description']); ?></p>
                    <a href="<?php echo e(route('front.blog.show', $card['slug'])); ?>" class="mt-5 inline-flex items-center gap-2 border border-[#8e8582] rounded-[4px] px-4 py-2 text-sm sm:text-base leading-none text-[#2f2725] <?php echo e($isJewellery ? 'bg-[#4b2a76] hover:bg-[#5a348a] text-[#d3b4d8] border-[#a47dab]/50' : 'bg-white hover:bg-[#f7f7f7]'); ?> transition-colors">
                        Read More
...
<?php /**PATH D:\My projects\CP\avnee\avnee_files\resources\views/partials/front/blog-section.blade.php ENDPATH**/ ?>