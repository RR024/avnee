...
<section class="bg-[#f6cfe0] min-h-screen text-[#1f1f1f] border-t border-[#7f2458]">
  <div class="max-w-[1320px] mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
    <div class="text-[10px] uppercase tracking-[0.16em] text-[#8b3c73] mb-4">
      <a href="<?php echo e(route('front.home')); ?>" class="hover:underline">Home</a>
      <span class="mx-1">/</span>
      <a href="<?php echo e(route('front.products.collection', ['collection' => 'best-sellers'])); ?>" class="hover:underline">Festive Wear</a>
      <span class="mx-1">/</span>
      <span class="text-[#6e2d5f]"><?php echo e($product['title']); ?></span>
    </div>
    ...
    <div class="grid gap-8 lg:gap-10 lg:grid-cols-[520px_minmax(0,1fr)]">
      <div>
        <div class="border border-[#d8b0c5] bg-white overflow-hidden mb-3">
          <img src="<?php echo e(asset('images/coupons/avnee7.jpeg')); ?>" alt="AVNEE7 Coupon Banner" class="w-full h-auto object-cover" />
        </div>
        <div class="relative border border-[#d6a7bf] bg-white overflow-hidden">
          ...
          <img id="best-buy-main-image" src="<?php echo e($galleryImages[0]); ?>" alt="<?php echo e($product['title']); ?>" class="w-full h-[560px] object-cover" />
          <span id="gallery-counter" class="absolute right-2 bottom-2 text-[11px] bg-black/55 text-white px-2 py-0.5">1 / <?php echo e(count($galleryImages)); ?></span>
        </div>
        <div class="mt-3 grid grid-cols-5 gap-2">
          <?php $__currentLoopData = $galleryImages; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $idx => $image): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <button type="button" class="best-buy-thumb border border-[#d6a7bf] bg-white p-0.5 <?php echo e($idx === 0 ? 'ring-1 ring-[#8b3c73]' : ''); ?>" data-image="<?php echo e($image); ?>" aria-label="thumbnail <?php echo e($idx + 1); ?>">
              <img src="<?php echo e($image); ?>" alt="<?php echo e($product['title']); ?> thumbnail <?php echo e($idx + 1); ?>" class="w-full h-20 object-cover" />
            </button>
      ...
      <div class="pt-1 max-w-[560px]">
        ...
        <h1 class="font-heading text-6xl leading-[0.92] text-[#231238] mt-2"><?php echo e(strtoupper($product['title'])); ?></h1>
        <p class="text-[14px] text-[#231238] mt-2 font-semibold"><?php echo e($product['title']); ?></p>
        <p class="text-[13px] text-[#8a5b78] mt-1">Style No <?php echo e($product['sku']); ?></p>

        <div class="mt-5 flex items-center gap-3">
          <span class="text-[46px] font-black leading-none text-[#1f1f2f]">₹<?php echo e(number_format($product['price'], 0)); ?></span>
        ...
        <div class="mt-5">
          ...
          <div class="flex flex-wrap gap-2.5" id="size-selector">
            <?php $__currentLoopData = ['0-6 Months','1-2 Years','2-3 Years','3-4 Years','4-5 Years','5-6 Years','6-7 Years','7-8 Years']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $size): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
              <button type="button" data-size="<?php echo e($size); ?>" class="size-btn px-4 py-2.5 border text-[14px] rounded-full <?php echo e($loop->first ? 'selected bg-black text-white border-black hover:bg-black' : 'border-[#d6a7bf] bg-white text-[#5f2d4f] hover:bg-[#ffeaf4]'); ?>"><?php echo e($size); ?></button>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
          </div>
        </div>
        ...
        <div class="mt-3 grid grid-cols-[80px_1fr] gap-2">
          ...
          <button type="button" class="h-11 bg-black text-white text-[14px] font-semibold uppercase tracking-[0.08em]">Add to Cart - ₹<?php echo e(number_format($product['price'], 0)); ?></button>
        </div>
        ...
        <p class="mt-5 text-[14px] text-[#6e4860]">10 people are currently looking at this product.</p>

        <div class="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2 text-[14px] text-[#2d1126]">
          <div class="flex items-center gap-2">
            <svg class="w-4 h-4 text-[#7f4a6a]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M12 8c-2.21 0-4 1.79-4 4s1.79 4 4 4 4-1.79 4-4-1.79-4-4-4z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M3.05 11a9 9 0 0117.9 0"/></svg>
            <span>100% Purchase Protection</span>
          </div>
          <div class="flex items-center gap-2">
            <svg class="w-4 h-4 text-[#7f4a6a]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M12 8v4l3 3"/><circle cx="12" cy="12" r="9" stroke-width="1.8"/></svg>
            <span>5 Day easy returns*</span>
          </div>
          <div class="flex items-center gap-2">
            <svg class="w-4 h-4 text-[#7f4a6a]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M12 2l2.7 5.47 6.03.88-4.36 4.25 1.03 6-5.4-2.84-5.4 2.84 1.03-6L3.27 8.35l6.03-.88L12 2z"/></svg>
            <span>Assured Quality</span>
          </div>
        </div>

        <div class="mt-6 border-t border-[#d8b0c5]">
          <?php $__currentLoopData = ['Product Details' => $product['description'], 'Style & Fit Tips' => 'Comfort fit with breathable fabric for all-day wear.', 'Shipping & Returns' => 'Ships in 2-4 business days. Easy returns within 5 days.', 'FAQs' => 'Need help with sizing? Contact our concierge team.']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $title => $content): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <details class="group border-b border-[#d8b0c5]">
              <summary class="flex items-center justify-between py-4 cursor-pointer list-none text-[15px] sm:text-[16px] font-semibold tracking-tight text-[#2d1126]">
                <span class="leading-none"><?php echo e($title); ?></span>
                <span class="text-[24px] leading-none text-[#5f2d4f] transition-transform duration-200 group-open:rotate-45">+</span>
              </summary>
              <p class="pb-4 text-[15px] text-[#5f2d4f]"><?php echo e($content); ?></p>
            </details>
          <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </div>
      </div>
    ...
    <div class="mt-14 bg-[#f6cfe0] border-t border-[#d8b0c5] py-12 w-screen relative left-1/2 -translate-x-1/2">
      <div class="w-full max-w-[1520px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="relative">
          ...
          <div class="swiper similar-products-swiper">
            <div class="swiper-wrapper">
              <?php $__currentLoopData = $similarProducts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="swiper-slide">
                  <a href="<?php echo e(route('front.best-buys.show', $item['slug'])); ?>" class="bg-white border border-[#d6a7bf] block">
                    <img src="<?php echo e($item['image']); ?>" alt="<?php echo e($item['title']); ?>" class="w-full h-52 object-cover" />
                    <div class="p-2">
                      <p class="text-[12px] font-semibold line-clamp-2 leading-tight"><?php echo e($item['title']); ?></p>
                      <p class="text-[13px] font-bold mt-1">₹<?php echo e(number_format($item['price'], 0)); ?></p>
                    </div>
                  </a>
                </div>
              <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        ...
        <div class="relative">
          ...
          <div class="swiper recently-viewed-swiper">
            <div class="swiper-wrapper">
              <?php $__currentLoopData = $recentlyViewed; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="swiper-slide">
                  <a href="<?php echo e(route('front.best-buys.show', $item['slug'])); ?>" class="bg-white border border-[#d6a7bf] block">
                    <img src="<?php echo e($item['image']); ?>" alt="<?php echo e($item['title']); ?>" class="w-full h-52 object-cover" />
                    <div class="p-2">
...
<?php echo $__env->make('layouts.front.' . (session('theme', 'studio')), array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\My projects\CP\avnee\avnee_files\resources\views/front/best-buy-detail.blade.php ENDPATH**/ ?>