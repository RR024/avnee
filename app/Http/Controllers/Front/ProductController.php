...
{
    public function studioKidsSale()
    {
    ...
    public function studioSareesSale()
    {
        return redirect()->route('front.products.index', [
            'collection' => 'sale',
            'discount' => 1,
    ...
    public function jewellerySale()
    {
        return redirect()->route('front.products.index', [
            'collection' => 'sale',
            'discount' => 1,
            'category' => 'jewellery-gallery',
            'force_theme' => 'jewellery',
        ]);
    }

    public function jewelleryAccessoriesSale()
    {
        return redirect()->route('front.products.index', [
            'collection' => 'sale',
            'discount' => 1,
    ...
    public function studioTrinketsSale()
    {
        return redirect()->route('front.products.index', [
            'collection' => 'sale',
            'discount' => 1,
    ...
    {
        $map = [
            'sale' => ['sort' => 'newest', 'discount' => 1, 'collection' => 'sale', 'collection_title' => 'Sale'],
            'new-arrivals' => ['sort' => 'newest', 'collection' => 'new-arrivals', 'collection_title' => 'New Arrivals'],
            'best-sellers' => ['sort' => 'price_desc', 'collection' => 'best-sellers', 'collection_title' => 'Best Sellers'],
            'bogo' => ['bogo' => 1, 'collection' => 'bogo', 'collection_title' => 'Buy One Get One'],
            'organizers' => ['sort' => 'newest', 'category' => 'organizers', 'collection' => 'organizers', 'collection_title' => 'Organizers'],
            'gifting' => ['sort' => 'newest', 'category' => 'gifting', 'collection' => 'gifting', 'collection_title' => 'Gifting'],
            'all-collections' => ['sort' => 'newest', 'collection' => 'all-collections', 'collection_title' => 'All Collections'],
            'party-frocks' => ['sort' => 'newest', 'category' => 'party-frocks', 'collection' => 'party-frocks', 'collection_title' => 'Party Frocks'],
            'summer-collections' => ['sort' => 'newest', 'category' => 'girls-dresses', 'collection' => 'summer-collections', 'collection_title' => 'Summer Collections'],
            'summer-classics' => ['sort' => 'newest', 'category' => 'girls-dresses', 'collection' => 'summer-classics', 'collection_title' => 'Summer Classics'],
            'birthday-glam' => ['sort' => 'newest', 'category' => 'party-frocks', 'collection' => 'birthday-glam', 'collection_title' => 'Birthday Glam'],
            'ugadi-sale' => ['sort' => 'newest', 'discount' => 1, 'category' => 'festive-wear', 'collection' => 'ugadi-sale', 'collection_title' => 'Ugadi Sale'],
            'festive-wear' => ['sort' => 'newest', 'category' => 'festive-wear', 'collection' => 'festive-wear', 'collection_title' => 'Festive Wear'],
            'daily-wear' => ['sort' => 'price_asc', 'category' => 'girls-dresses', 'collection' => 'daily-wear', 'collection_title' => 'Daily Wear'],
            'all-sarees' => ['sort' => 'newest', 'category' => 'sarees', 'collection' => 'all-sarees', 'collection_title' => 'All Sarees'],
            'printed-cotton' => ['sort' => 'newest', 'category' => 'sarees', 'collection' => 'printed-cotton', 'collection_title' => 'Printed Cotton'],
            'georgette' => ['sort' => 'newest', 'category' => 'sarees', 'collection' => 'georgette', 'collection_title' => 'Georgette'],
            'semi-silk' => ['sort' => 'newest', 'category' => 'sarees', 'collection' => 'semi-silk', 'collection_title' => 'Semi Silk'],
            'daily-wear-sarees' => ['sort' => 'newest', 'category' => 'sarees', 'collection' => 'daily-wear-sarees', 'collection_title' => 'Daily Wear Sarees'],
            'cotton-sarees' => ['sort' => 'newest', 'category' => 'sarees', 'collection' => 'cotton-sarees', 'collection_title' => 'Cotton Sarees'],
        ...
        // Merge collection config into query and redirect to products index
        $query = array_merge($request->query(), $collectionConfig);
    ...
    {
        $forcedTheme = $request->query('force_theme');
        if (in_array($forcedTheme, ['studio', 'jewellery'], true)) {
            session(['theme' => $forcedTheme, 'brand_id' => $forcedTheme === 'jewellery' ? 2 : 1]);
        }
        ...
        $jewelleryCategorySlugs = [
            ...
            'korean', 'traditional', 'kundan', 'oxidised', '18k-gold-plated', 'fashion', 'watches'
        ];

        if (!$forcedTheme && $request->filled('category') && in_array($request->query('category'), $studioOnlyCategorySlugs, true)) {
            session(['theme' => 'studio', 'brand_id' => 1]);
        }

        if (!$forcedTheme && $request->filled('category') && in_array($request->query('category'), $jewelleryCategorySlugs, true)) {
            session(['theme' => 'jewellery', 'brand_id' => 2]);
        }
    ...
    public function showPartyFrockDetail(string $product)
    {
        ...
        if (session('theme', 'studio') === 'jewellery' && $linkedProduct) {
            $productModel = Product::whereKey($linkedProduct->id)
                ->with(['images', 'variants', 'category', 'reviews' => function($q) {
                    $q->where('status', 'approved')->with('user')->latest();
                }, 'flashSales' => function($q) {
                    $q->where('is_active', true)
            ...
            $relatedProducts = Product::where('category_id', $productModel->category_id)
            ...
            $recentlyViewedIds = collect(session('recently_viewed_products', []))
                ->map(fn($id) => (int) $id)
                ->filter(fn($id) => $id > 0)
                ->reject(fn($id) => $id === (int) $productModel->id)
                ->prepend((int) $productModel->id)
                ->unique()
                ->values();
    ...
    {
        $catalogConfigs = $this->getStyleCatalogConfigs();
        $categoryInCatalog = array_key_exists($category, $catalogConfigs);

        // -------------------------------------------------------------------
        // Handle bogo-sample-N slugs for ANY category (including kids size
        // categories like 2-4-years, 6-14-years that aren't in folder catalogs)
        // -------------------------------------------------------------------
        if (str_starts_with($product, 'bogo-sample-')) {
            $index = (int) substr(str_replace('bogo-sample-', '', $product), 0) - 1;
            $sampleGallery = $this->buildCrossThemeSampleGallery(20);

            if (isset($sampleGallery[$index])) {
                $categoryLabel = $categoryInCatalog
                    ? $catalogConfigs[$category]['label']
                    : ucwords(str_replace('-', ' ', $category));

                $galleryProduct = [
                    'slug'        => $product,
                    'title'       => $sampleGallery[$index]['title'] ?? ($categoryLabel . ' Sample ' . ($index + 1)),
                ...
                $theme = session('theme', 'studio');
                $catalog = $categoryInCatalog ? $this->buildStyleCatalog($catalogConfigs[$category]) : [];
                $linkedProduct = $this->ensureCartReadyProductForGalleryItem($galleryProduct, $category, $categoryLabel);
                $detailContent = $this->buildGalleryDetailContent($galleryProduct, $categoryLabel, $linkedProduct);
                $similarGalleryProducts = !empty($catalog) ? $this->buildSimilarGalleryProducts($catalog, $galleryProduct, $category, 8) : [];
                $recentlyViewedGalleryProducts = !empty($catalog) ? $this->buildRecentlyViewedGalleryProducts($catalog, $galleryProduct, $category, 8) : [];

                if ($theme === 'jewellery' && $linkedProduct) {
                    $productModel = Product::whereKey($linkedProduct->id)
                        ->with(['images', 'variants', 'category', 'reviews' => function($q) {
                            $q->where('status', 'approved')->with('user')->latest();
                        }, 'flashSales' => function($q) {
                    ...
                    $recentlyViewedIds = collect(session('recently_viewed_products', []))
                        ...
                        ->reject(fn($id) => $id === (int) $productModel->id)
                        ->prepend((int) $productModel->id)->unique()->values();
                    session(['recently_viewed_products' => $recentlyViewedIds->take(12)->all()]);

                    $displayIds = $recentlyViewedIds->reject(fn($id) => $id === (int) $productModel->id)->take(8)->values();
                    $recentlyViewedProducts = collect();
                    if ($displayIds->isNotEmpty()) {
                        $byId = Product::whereIn('id', $displayIds->all())->where('is_active', true)
                            ->where('brand_id', $productModel->brand_id)->get()->keyBy('id');
                        $recentlyViewedProducts = $displayIds->map(fn($id) => $byId->get($id))->filter()->values();
                    }
                    $product = $productModel;
                    return view('front.product.detail', compact('product', 'relatedProducts', 'theme', 'recentlyViewedProducts'));
                }
