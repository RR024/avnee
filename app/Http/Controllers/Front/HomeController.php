...
class HomeController extends Controller
{
    ...
    public function summerClassicsCampaign()
    {
        session(['theme' => 'studio', 'brand_id' => 1]);

        $products = $this->campaignProductsByCategory('girls-dresses');

        return view('front.campaign', [
            'campaignTitle' => 'Summer Classics',
            'campaignSubtitle' => 'Light, breezy, and joyful styles for sunshine days.',
            'campaignImage' => asset('images/hero-slider/summer-classics.png'),
            'products' => $products,
        ]);
    }

    public function birthdayGlamCampaign()
    {
        session(['theme' => 'studio', 'brand_id' => 1]);

        $products = $this->campaignProductsByCategory('party-frocks');

        return view('front.campaign', [
            'campaignTitle' => 'Birthday Glam',
            'campaignSubtitle' => 'Statement party frocks made for celebration moments.',
            'campaignImage' => asset('images/hero-slider/birthday.png'),
            'products' => $products,
        ]);
    }

    public function ugadiSaleCampaign()
    {
        session(['theme' => 'studio', 'brand_id' => 1]);

        $products = $this->campaignProductsByCategory('festive-wear', true);

        return view('front.campaign', [
            'campaignTitle' => 'Ugadi Sale',
            'campaignSubtitle' => 'Festive picks with special sale pricing.',
            'campaignImage' => asset('images/hero-slider/ugadi-sale.png'),
            'products' => $products,
        ]);
    }

    private function campaignProductsByCategory(string $slug, bool $discountOnly = false)
    {
        $query = Product::query()
            ->where('is_active', true)
            ->where('brand_id', 1)
            ->where(function ($q) use ($slug) {
                $q->whereHas('category', function ($cat) use ($slug) {
                    $cat->where('slug', $slug);
                })->orWhereHas('subcategory', function ($sub) use ($slug) {
                    $sub->where('slug', $slug);
                });
            })
            ->latest();
        ...
        if ($products->isEmpty() && $discountOnly) {
