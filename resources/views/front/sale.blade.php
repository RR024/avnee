    ...
    @elseif(!empty($saleFallbackCards))
      <div class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-x-5 gap-y-8">
        @foreach($saleFallbackCards as $card)
          <div class="group relative flex flex-col">
            <a href="{{ route('front.products.index', !empty($card['collection']) ? ['collection' => $card['collection']] : []) }}" class="relative block overflow-hidden bg-white aspect-[3/4] border border-[#C75B6E] rounded-lg">
              <img src="{{ $card['image'] }}"
                   alt="{{ $card['title'] }}"
                   class="w-full h-full object-cover object-top transition-transform duration-700 group-hover:scale-105" />
              <div class="absolute top-2 right-2 inline-flex items-center overflow-hidden rounded-sm shadow-md">
            ...
            <div class="pt-4">
              <h3 class="text-sm font-semibold text-[#C75B6E] leading-snug line-clamp-2 uppercase">{{ $card['title'] }}</h3>
              <p class="text-xs text-[#C75B6E] mt-1">{{ $card['label'] }}</p>
            </div>
          </div>
        @endforeach
      </div>
