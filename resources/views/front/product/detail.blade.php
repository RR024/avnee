...
<div class="{{ $bgColor }} selection:bg-black selection:text-white transition-colors duration-700">
    <div class="max-w-[1320px] mx-auto px-4 sm:px-6 lg:px-10 pt-8 pb-20">
        ...
        <section class="mb-6 border {{ $borderColor }} bg-white overflow-hidden">
            <img src="{{ $couponBanner }}" alt="AVNEE COLLECTIONS Coupon Banner" class="w-full h-auto object-cover" />
        </section>
        @endif
        ...
        <div class="grid grid-cols-1 xl:grid-cols-[72px_minmax(0,1fr)_360px] gap-5 xl:gap-7 items-start">
            ...
            <div class="order-1 xl:order-2">
                <div class="relative bg-[#d8d8d8] border {{ $borderColor }} w-full max-w-[640px] xl:max-w-[700px] mx-auto xl:mx-0 min-h-[390px] sm:min-h-[520px] xl:min-h-[590px] flex items-center justify-center overflow-hidden">
                    <img id="main-product-image" src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->name }}" class="max-h-[92%] max-w-[92%] object-contain transition-all duration-500 ease-out" />
                    @if($activeDiscount > 0)
                ...
                <form action="{{ route('cart.add') }}" method="POST" id="add-to-cart-form">
                    ...
                    <div class="mb-6 pt-2">
                        <div class="grid grid-cols-2 gap-x-6 gap-y-3 text-[12px] leading-snug {{ $mutedColor }}">
                            ...
                            @if($product->show_returnable_badge ?? true)
                            ...
                            @endif
                            @if($product->show_free_shipping_badge ?? true)
        ...
        <div class="mt-16 pt-10 border-t {{ $borderColor }}">
            <h2 class="font-heading text-2xl sm:text-3xl {{ $isDark ? 'text-center' : 'text-left' }} font-normal tracking-tight uppercase mb-8 {{ $isDark ? 'text-[#f3d9ff]' : 'text-gray-900' }}">Similar Products</h2>
            ...
            <div class="grid grid-cols-2 lg:grid-cols-5 gap-5">
                ...
                <div class="group relative flex flex-col">
                    ...
                    <div class="space-y-2">
                        <a href="{{ route('front.product.detail', $related->slug ?? $related->id) }}" class="block text-[11px] font-bold tracking-widest uppercase {{ $isDark ? 'text-[#f3d9ff]' : 'text-gray-800' }} line-clamp-1 opacity-80 group-hover:opacity-100 transition-opacity">
                            {{ $related->name }}
                        </a>
                        <div class="flex items-center gap-3">
                            <span class="text-sm font-bold tracking-tight {{ $isDark ? 'text-[#d4af37]' : 'text-gray-900' }}">₹{{ number_format($related->price, 0) }}</span>
                            @if($related->compare_price > $related->price)
                                <span class="text-[10px] {{ $mutedColor }} line-through uppercase opacity-50">₹{{ number_format($related->compare_price, 0) }}</span>
                            @endif
                        </div>
                    </div>
                </div>
        ...
        <section class="mt-14 pt-10 border-t {{ $borderColor }}">
            <h2 class="font-heading text-2xl sm:text-3xl {{ $isDark ? 'text-center' : 'text-left' }} font-normal tracking-[0.16em] uppercase mb-8 {{ $isDark ? 'text-[#f3d9ff]' : 'text-gray-900' }}">Recently Viewed</h2>
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-5">
                @foreach($recentlyViewedProducts as $recent)
                    <a href="{{ route('front.product.detail', $recent->slug ?? $recent->id) }}" class="group block">
                        ...
                        <p class="mt-3 text-[11px] font-bold tracking-[0.12em] uppercase {{ $isDark ? 'text-[#f3d9ff]' : 'text-gray-800' }} line-clamp-1">{{ $recent->name }}</p>
                        <p class="text-sm font-semibold {{ $isDark ? 'text-[#d4af37]' : 'text-gray-900' }}">₹{{ number_format($recent->price, 0) }}</p>
                    </a>
                @endforeach
            </div>
        </section>
